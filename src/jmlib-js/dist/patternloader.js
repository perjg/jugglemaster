...
var patternLibrary = require('./patterns');
...
PatternLoader.loadStyle = function (jmlib, name) {
	var style = patternLibrary.styles[name];
...
PatternLoader.loadPatternEx = function (jmlib, index) {
	var pattern = patternLibrary.patterns[index];
...
PatternLoader.loadPattern = function (jmlib, name) {
	for (var i = 0; i < patternLibrary.patterns.length; i++) {
		var pattern = patternLibrary.patterns[i];

		if (pattern === null) {
			return false;
		}

		if (pattern[0] === name) return PatternLoader.loadPatternEx(jmlib, i);
	}
