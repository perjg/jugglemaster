...
JMPatternLoader::JMPatternLoader(FILE* file) {
  patternfile = file;
}
...
void JMPatternLoader::setCategory(char* name) {
  ...
  curCatPos = (int)categories.GetCount() - 1; // zero-indexed
}
...
int JMPatternLoader::getCategoryCount(void) {
  return (int)categories.GetCount();
}

int JMPatternLoader::getPatternCount(void) {
  return (int)patterns.GetCount();
}
